# Test Suite Configuration Usage: cnti-testsuite.yaml
### What is the cnti-testsuite.yaml and why is it required?

The cnti-testsuite.yaml is used by `cnf_install` in order to install the CNF to be tested onto an existing K8s cluster. 


The information in the cnti-testsuite.yaml is also used for additional configuration of some tests e.g. `white_list_container_names` is used for excluding containers from the [privileged_containers](https://github.com/lfn-cnti/testsuite/blob/main/src/tasks/workload/security.cr#L138) container test.


### Table of Contents

- [Overview](#Overview-of-all-cnti-testsuite.yaml)
- [Generator Quick Start](#Generator-Quick-Start)
- [Keys and Values](#Keys-and-Values)


### Overview of all cnti-testsuite.yaml

The following is a basic working example cnti-testsuite.yaml file that can be found in the CNTi Test Suite repository: [/spec/fixtures/cnti-testsuite-v2-example.yaml](https://github.com/lfn-cnti/testsuite/blob/main/spec/fixtures/cnti-testsuite-v2-example.yaml)
...
cnti-testsuite.yaml can be dynamically generated by running `./cnti-testsuite generate_config`.
The process is interactive and should be friendly to new users.
...
New releases may change the format of cnti-testsuite.yaml. To update your older configs automatically to the latest version, use the `update_config` task.

```
./cnti-testsuite update_config input_config=OLD_CONFIG_PATH output_config=NEW_CONFIG_PATH
```
...
Common parameters are required for proper functioning of some tests. If these parameters are not set up, corresponding tests will be failed/skipped or would not function properly (for example - white_list_container_names).
`common: ...`
...
This value is used to allow particular hardcoded IPs to be exempted from the `hardcoded_ip_addresses_in_k8s_runtime_configuration` test when the CNF is being validated.
The reason this is needed is because the CNTi Test Suite checks all configuration files and manifests used by the CNF for the presence of hardcoded IP addresses.
...
When using a private registry hosted on the cluster, the image references in the CNF's helm chart may refer to the registry host with the Kubernetes service name alone. The CNTi Test Suite runs a docker daemon in a separate `cnti-testsuite` namespace on the Kubernetes cluster. So it is required for the testsuite to be aware of the FQDN of the service, along with the port.

Use this option to configure FQDNs of image registries for the testsuite to access.

If the CNF's helm charts use the image url `foobar:5000/hello:latest`, then please use the configuration below in `cnti-testsuite.yaml` to provide the FQDN mapping for the image registry.
...
deployments:
  helm_dirs:
    - name: envoy                     # Name of the deployment
      ...
      helm_values: --set myvalue=42   # Additional values that would be used for helm installation
      namespace: cnti-default          # Namespace to which deployment would be installed (cnti-default is default)
```
