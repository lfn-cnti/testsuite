...
The information in the cnf-testsuite.yml is also used for additional configuration of some tests e.g. `white_list_container_names` is used for excluding containers from the [privileged_containers](https://github.com/lfn-cnti/testsuite/blob/main/src/tasks/workload/security.cr#L138) container test.
...
The following is a basic working example cnf-testsuite.yml file that can be found in the CNTi Test Suite repository: [/spec/fixtures/cnf-testsuite-v2-example.yml](https://github.com/lfn-cnti/testsuite/blob/main/spec/fixtures/cnf-testsuite-v2-example.yml)
...
If the CNF's helm charts use the image url `foobar:5000/hello:latest`, then please use the configuration below in `cnf-testsuite.yml` to provide the FQDN mapping for the image registry.
