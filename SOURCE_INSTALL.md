## Installing the CNTI Test Suite from Source

### Overview

This INSTALL guide will detail the minimum requirements needed for cnti-testsuite to install from source.
...
- **curl**
- **helm 3.8.2** _or newer_ (helm 4 supported; cnti-testsuite installs helm 4 if not found locally)
...
We can assume you have access to a working kubernetes cluster. We recommend only running the cnti-testsuite on dev or test clusters. The source install steps have been verified on most Linux distributions (Ubuntu, Debian and CentOS), Mac OS X and WSL as long as crystal-lang >=v1.6.0 is installed.
...
- cnti-testsuite needs helm 3.8.2 or greater (helm 4 supported) but it is optional as the prerequisite checks will install if not found. You can install helm by checking their [installation methods](https://helm.sh/docs/helm/helm_install/).
...
- Change directory into the source:
  ...
  cd cnti-testsuite
  ```
- Now we need to run shards to pull down requirements needed to build and compile cnti-testsuite:
  ```
  shards install
  ```
- Now build a cnti-testsuite binary (this method will have runtime dependencies but should not pose any issues):
  ```
  crystal build src/cnti-testsuite.cr
  ```
  This should create an executable `cnti-testsuite` binary in your source checkout.

<details><summary>(Optional) Build cnti-testsuite using Docker  Alpine Image</summary>
<p>
...
docker pull crystallang/crystal:1.19.1-alpine-build
 ## Note: $PWD is your cnti-testsuite source directory in the following commands 
docker run --rm -it -v $PWD:/workspace -w /workspace crystallang/crystal:1.19.1-alpine-build shards install
docker run --rm -it -v $PWD:/workspace -w /workspace crystallang/crystal:1.19.1-alpine-build crystal build src/cnti-testsuite.cr --release --static --link-flags "-lxml2 -llzma"
```
...
You will need to have cnti-testsuite executable in your current PATH for this to work properly.

```
./cnti-testsuite completion -l error > test.sh
source test.sh
```
...
<p>
The CNTI Test Suite is modeled after make, or if you're familiar with Ruby, [rake](https://github.com/ruby/rake). Tests are created via tasks using the Crystal library, [SAM.cr](https://github.com/imdrasil/sam.cr).
...
Now that we have a `cnti-testsuite` binary, we can run `setup` to ensure it has all the pre-requisites needed in order to successfully run tests and prepare the required `cnti/` workspace directory and other files required for cnti-testsuite.

- Run the following to prepare cnti-testsuite:
  ```
...
Now that cnti-testsuite is installed and prepared, we can now run CNF workloads and tests. We recommend installing and running a sample CNF to ensure cnti-testsuite is operational and set expectations of the output.

#### Configuring an example CNF

To use CoreDNS as an example CNF. Download the testsuite configuration to test CoreDNS:

- Make sure you are in your cnti-testsuite/ source repo checkout directory and do the following:
  ```
...
- Prepare the test suite to use the CNF by running:
  ...
  ```
  # via crystal
  crystal src/cnti-testsuite.cr cnf_install --cnf-config ./cnti-testsuite.yaml
  ```
...
- To support auto deployment of the CNF from the ([cnti-testsuite.yaml](CNTI_TESTSUITE_YAML_USAGE.md)) configuration file.
...
You can also run via `crystal` by replacing the `./cnti-testsuite` with `crystal spec src/cnti-testsuite.cr` and then the argument.

#### More Example Usage (also see the [complete usage documentation](https://github.com/lfn-cnti/testsuite/blob/main/USAGE.md))

```
# These assume you've already run the cnf_install pointing at a cnti-testsuite.yaml config above. You can always specify your config at the end of each command as well, eg:
./cnti-testsuite all --cnf-config <path to your config yml>/cnti-testsuite.yaml

# Runs all ga tests (generally available workload and platform tests)
./cnti-testsuite all

# Runs all alpha, beta and ga tests
./cnti-testsuite all --alpha

# Runs all beta and ga tests
./cnti-testsuite all --beta

# Run all wip, alpha, beta, and ga tests
./cnti-testsuite all --wip

# Run all tests in the configuration lifecycle category
./cnti-testsuite configuration_lifecycle

# Run all tests in the installability
./cnti-testsuite installability
```
...
You can check out our [CNTI_TESTSUITE_YAML_USAGE.md](https://github.com/lfn-cnti/testsuite/blob/main/CNTI_TESTSUITE_YAML_USAGE.md) document on what is required to bring or use your own CNF.

- Follow the [INSTALL](INSTALL.md) or [SOURCE-INSTALL](SOURCE-INSTALL.md) to build the binary.
- Now head over to [CNTI_TESTSUITE_YAML_USAGE.md](https://github.com/lfn-cnti/testsuite/blob/main/CNTI_TESTSUITE_YAML_USAGE.md) for more detailed steps.
