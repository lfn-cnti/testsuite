...
scored_task "smf_upf_core_validator",
...
scored_task "suci_enabled",
  ...
  CNFManager::Task.task_runner(args, task: t) do |args, config, result|
    ...
    if core
      ...
      suci_found = false
      result.append_remediation("You must set the core label for you AMF node")
    end
