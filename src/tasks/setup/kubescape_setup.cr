...
namespace "setup" do
  ...
  task "install_kubescape", ["setup:kubescape_framework_download"] do |_, args|
    ...
    begin
      download(Setup::KUBESCAPE_URL, kubescape_binary)
    rescue ex : Exception
  ...
  task "kubescape_framework_download" do |_, args|
    ...
    if File.exists?("#{Setup::KUBESCAPE_DIR}/nsa.json") && installed_framework_version == Setup::KUBESCAPE_FRAMEWORK_VERSION
