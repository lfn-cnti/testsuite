...
task "install_opa", ["setup:helm_local_install", "setup:create_namespace"] do |_, args|
  ...
  KubectlClient::Wait.wait_for_install_by_apply("#{MANIFESTS_DIR}/constraint_template.yml")
  KubectlClient::Wait.wait_for_condition("crd", "requiretags.constraints.gatekeeper.sh", "condition=established", 300)
  KubectlClient::Apply.file("#{MANIFESTS_DIR}/enforce-image-tag.yml")
end
...
task "uninstall_opa" do |_, args|
  ...
  begin KubectlClient::Delete.file("#{MANIFESTS_DIR}/enforce-image-tag.yml") rescue KubectlClient::ShellCMD::NotFoundError end
  begin KubectlClient::Delete.file("#{MANIFESTS_DIR}/constraint_template.yml") rescue KubectlClient::ShellCMD::NotFoundError end
end
