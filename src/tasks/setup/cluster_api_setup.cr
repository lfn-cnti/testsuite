...
task "install_cluster_api" do |_, args|
  ...
  if Dir.exists?(Setup::CLUSTER_API_DIR)
    logger.notice { "cluster api directory: '#{CLUSTER_API_DIR}' already exists, clusterctl should be available" }
    next
  end

  FileUtils.mkdir_p(CLUSTER_API_DIR)
  begin
  ...
  File.write("#{CLUSTER_API_DIR}/clusterctl.yaml", "CLUSTER_TOPOLOGY: \"true\"")
