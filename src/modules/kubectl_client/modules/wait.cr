  ...
  module Wait
    ...
    def self.resource_wait_for_uninstall(
      ...
      while seconds <= wait_count
        ...
        # Step 1: check whether the root resource still exists
        if root_alive
          begin
        ...
        # Step 2: once the root is gone, prune descendants by UID
        unless root_alive
          pending.clone.each do |descendant|
            begin
              data = KubectlClient::Get.resource(
                descendant[:kind],
                nil,                          # no specific name
                descendant[:namespace],
                descendant[:namespace].nil?,  # all_namespaces if namespace is nil
              )
