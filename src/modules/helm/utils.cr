...
module Helm
  ...
  def self.install_local_helm : Bool
    ...
    begin
      ...
      download(Setup::HELM_URL, helm_archive)
    rescue ex : Exception
      logger.error { "Error while downloading Helm binary: #{ex.message}" }
      return false
    end

    unless TarClient.untar(helm_archive, Setup::HELM_DIR)[:status].success?
      logger.error { "Error while extracting Helm binary: #{ex.message}" }
      return false
    end
  end
