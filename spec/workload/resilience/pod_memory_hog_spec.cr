...
describe "Resilience pod memory hog Chaos" do
  ...
  it "'pod_memory_hog' A 'Good' CNF should not crash when pod memory hog occurs", tags: ["pod_memory_hog"]  do
    begin
      ...
      (/> Litmus pod-memory-hog on Deployment\/coredns-coredns in cnti-default: verdict Pass; target (deployment coredns-coredns|pod coredns-coredns-\S+) \((targeted|reverted)\)/ =~ result[:output]).should_not be_nil
