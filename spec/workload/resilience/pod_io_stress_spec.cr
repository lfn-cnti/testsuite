...
describe "Resilience pod delete Chaos" do
  ...
  it "'pod_io_stress' A 'Good' CNF should not crash when pod delete occurs", tags: ["pod_io_stress"]  do
    begin
      ...
      (/> Litmus pod-io-stress on Deployment\/coredns-coredns in cnti-default: verdict Pass; target (deployment coredns-coredns|pod coredns-coredns-\S+) \((targeted|reverted)\)/ =~ result[:output]).should_not be_nil
