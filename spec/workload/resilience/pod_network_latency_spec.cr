...
describe "Resilience Pod Network Latency Chaos" do
  ...
  it "'pod_network_latency' A 'Good' CNF should not crash when network latency occurs", tags: ["pod_network_latency"]  do
    begin
      ...
      (/> Litmus pod-network-latency on Deployment\/coredns-coredns in cnti-default: verdict Pass; target (deployment coredns-coredns|pod coredns-coredns-\S+) \((targeted|reverted)\)/ =~ result[:output]).should_not be_nil
      verify_task_result("pod_network_latency", "passed")
    rescue ex
