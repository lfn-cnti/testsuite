...
describe CnfTestSuite do
  ...
  it "'liveness' should pass when livenessProbe is set", tags: ["liveness"] do
    begin
      ...
      (/(PASSED).*(All workload resources have at least one container with a livenessProbe)/ =~ result[:output]).should_not be_nil
    ensure
  ...
  it "'liveness' should fail when livenessProbe is not set", tags: ["liveness"] do
    begin
      ...
      result[:status].success?.should be_true
      (/(FAILED).*(One or more workload resources have no containers with a livenessProbe)/ =~ result[:output]).should_not be_nil
    ensure
      result = ShellCmd.cnf_uninstall()
    end
  end
  ...
  it "'readiness' should pass when readinessProbe is set", tags: ["readiness"] do
    begin
      ...
      result[:status].success?.should be_true
      (/(PASSED).*(All workload resources have at least one container with a readinessProbe)/ =~ result[:output]).should_not be_nil
    ensure
      result = ShellCmd.cnf_uninstall()
    end
  end
  ...
  it "'readiness' should fail when readinessProbe is not set", tags: ["readiness"] do
    begin
      ...
      result[:status].success?.should be_true
      (/(FAILED).*(One or more workload resources have no containers with a readinessProbe)/ =~ result[:output]).should_not be_nil
    ensure
      result = ShellCmd.cnf_uninstall()
    end
  end
